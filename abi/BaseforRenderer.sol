...
/**
 * @title BaseforRenderer
 * @notice Builds HTML animation for Basefor NFTs
 * @dev Uses BaseforAssets for all static base64 assets
 *      Only phrases and textColors are dynamic per NFT
 */
contract BaseforRenderer {
    IBaseforAssets public immutable assets;
    ...
    /**
     * @notice Build complete HTML for NFT animation
    ...
    function _buildHead() internal view returns (string memory) {
        return
            string(
                abi.encodePacked(
                    '<html lang="en"><head>',
                    ...
                    assets.getCSS(),
                    "</style></head>",
                    '<body style="font-family:-apple-system,BlinkMacSystemFont,sans-serif;margin:0;padding:0;">',
                    '<div class="container" style="width:512px;height:512px;background-color:#fff;display:flex;flex-direction:column;justify-content:center;align-items:center;overflow:hidden;">'
                )
            );
    }
    ...
    function _buildBody(
        ...
        return
            string(
                abi.encodePacked(
                    '<canvas id="c" width="512" height="512" style="position:absolute;top:0;left:0;z-index:1;"></canvas>',
                    '<canvas id="hidden" style="display:none;"></canvas>',
                    '<h1 style="margin:0;padding:0;font-size:28px;text-align:center;position:relative;z-index:10;">',
                    _buildLogoAndTitle(),
                    _buildPhrase(
                        phrase1,
                        phrase2,
    ...
    function _buildLogoAndTitle() internal pure returns (string memory) {
        return
            string(
                abi.encodePacked(
                    '<span style="display:inline-flex;align-items:center;gap:0">',
                    '<svg width="70.2" height="28" viewBox="0 0 351 90" fill="none" xmlns="http://www.w3.org/2000/svg">',
    ...
    function _buildPhrase(
        ...
        return
            string(
                abi.encodePacked(
                    '<div style="margin:0;padding:0;height:28px;line-height:28px;font-size:24px;text-transform:lowercase;overflow:hidden;text-align:center;">',
                    '<span style="position:relative;animation:animation 10s ease infinite">',
                    '<span style="color:',
                    textColor1,
                    '">',
                    phrase1,
    ...
    function _buildScript() internal view returns (string memory) {
        return
            string(
                abi.encodePacked(
                    "<script>const cloudSVG='",
                    assets.getCloudSVGBase64(),
                    "';",
    ...
    /**
     * @notice Get assets contract address for client-side assembly
     */
