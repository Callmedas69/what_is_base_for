 ...
 * @notice Builds pure SVG animation for Basefor NFTs
 * @dev Uses BaseforAssets for CSS keyframes and embedded Doto font
 */
contract BaseforRenderer {
        ...
        return
            string(
                abi.encodePacked(
                    ...
                    _buildContent(phrase1, phrase2, phrase3, textColor1, textColor2, textColor3),
                    '</svg>'
                )
            );
    }
    ...
    function _buildContent(
        ...
        return
            string(
                abi.encodePacked(
                    '<g transform="translate(256,256)" text-anchor="middle" font-family="-apple-system, BlinkMacSystemFont, Segoe UI, Roboto, sans-serif">',
                    _buildLogo(),
                    '<text x="40" y="5" font-size="38" font-weight="500">.is</text>',
    ...
    function _buildLogo() internal pure returns (string memory) {
        return
            string(
                abi.encodePacked(
                    '<g transform="translate(-80,-20) scale(0.28)">',
    ...
    function _buildPhrases(
        ...
        return
            string(
                abi.encodePacked(
                    '<svg x="-240" y="10" width="480" height="45" overflow="hidden" xmlns="http://www.w3.org/2000/svg">',
                    "<g class=\"rotator\" font-weight=\"700\" font-size=\"42\" font-family=\"'Doto', sans-serif\" text-anchor=\"middle\">",
                    '<text x="240" y="35" fill="', textColor1, '">', phrase1, '</text>',
                    '<text x="240" y="80" fill="', textColor2, '">', phrase2, '</text>',
                    '<text x="240" y="125" fill="', textColor3, '">', phrase3, '</text>',
                    '</g></svg>'
                )
            );
    }
